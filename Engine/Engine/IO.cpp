...
void IO::genMaze(Bin<EntityPhysics>* bin, EntityPhysics e, unsigned int seed)
{
	...
	for (int ring = 0; ring < nodes/2; ++ring) {	//For each ring (starting at outer ring going inwards)
		...
		for (int i = ring + 1; i < aring; ++i) {
			randomPassage(node, i + ring*nodes, 0, 1, 1, 1);	//North row
			randomPassage(node, aring + i*nodes, 1, 0, 1, 1);	//East row
			randomPassage(node, i + aring*nodes, 1, 1, 0, 1);	//South row
			randomPassage(node, ring + i*nodes, 1, 1, 1, 0);	//West row
		}
	...
	for (int i = 0; i < MAP_SIZE; ++i) {
		for (int j = 0; j < MAP_SIZE; ++j) {
			if (map[i + j*MAP_SIZE]) {
				...
				temp->move(Vector3(i*e.getScale().x*2 , 0, j*e.getScale().z * 2));
